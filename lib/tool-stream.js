...
ToolStream.prototype.collectMatch = function(property, match) {
  ...
  function transform(obj, enc, next) {
    var self = this
    if (tool.getValue(obj, property).match(match)) { self.push(obj) }
    next()
  }
}
...
// Only pass value from a specified key in object
ToolStream.prototype.insertProperty = function(property, value) {
  return through.obj(transform)
  function transform(obj, enc, next) {
    this.push(tool.setValue(obj, property, value))
    next()
  }
}
...
ToolStream.prototype.setValue = function(obj, path, newvalue) {
  var value = obj
  var parent
  path.split('.').forEach(moveDeeper)
  function moveDeeper(key) {
